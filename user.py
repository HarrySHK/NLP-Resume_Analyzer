    ...
    if pdf_file is not None:
       ...
       #     X = vectorizer.fit_transform(df['skills'])
         ...
         #   clf = MultinomialNB().fit(X, df['job_role'])
            ...
            st.text("\n\n\n")
            store_resumedata_in_mongodb(resume_data['name'],resume_data['email'],resume_data['mobile_number'] ,clf ,resume_score,resume_data['total_experience'],timestamp, 'mongodb+srv://WajidAhmed:WAJID12345@cluster25.frhncjv.mongodb.net/?retryWrites=true&w=majority') 
           # st.balloons()
            st.snow()
